...
/**
 * v11: generations.kind CHECK 에 'nine_slice_trimmed' 추가
 * (9-slice 편집기 /api/nine-slice-trim 결과 kind — 모서리/엣지만 남기고 중앙을 2px 로 트림).
 *
 * SQLite 는 CHECK 변경에 ALTER 가 불가하므로 테이블 재생성 방식. 데이터 변형은
 * 없고 CHECK 제약만 확장하므로 straight copy 한다. 테이블 재생성 전 .bak-v11 백업.
 */
function migrateV11(db: Database.Database): void {
  const backupPath = DB_PATH + ".bak-v11";
  if (fs.existsSync(DB_PATH) && !fs.existsSync(backupPath)) {
    db.pragma("wal_checkpoint(TRUNCATE)");
    fs.copyFileSync(DB_PATH, backupPath);
  }
  ...
  try {
    db.exec(`
      ...
      CREATE TABLE generations_new (
        ...
        message_id        TEXT REFERENCES messages(id) ON DELETE SET NULL,
        kind              TEXT NOT NULL
                          CHECK(kind IN ('text2img','img2img','upscale','remove_bg','inpaint','spritesheet','mask','layer','layer_extract','external','reskin','resize','emote_sheet','tileset','normal_map','composite','sprite_effect','nine_slice','nine_slice_scaled','button_state','nine_slice_trimmed')),
        prompt            TEXT,
        negative_prompt   TEXT,
        preset_id         TEXT REFERENCES style_presets(id) ON DELETE SET NULL,
        input_image_ids   TEXT,
