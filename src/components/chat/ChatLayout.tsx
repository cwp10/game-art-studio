...
import { chatReducer, friendlyError, initialState, type ChatItem } from "./chat-state";
...
export function ChatLayout() {
  ...
  // 스트림 전송 로직 — handleSend/handleBatch/handleCancel + 전용 ref(presetCache/abortRef) 캡슐화.
  const { handleSend, handleCancel } = useStreamChat({
    dispatch,
  ...
  const handleAskSuggestions = useCallback(
    async (text: string, attachedGenerationIds?: string[]) => {
      try {
        ...
        try {
          const items = await suggestPrompts(text, attachedGenerationIds);
...
const SEED_POOLS: string[][] = [
  // 캐릭터 — 스타일 다양화(픽셀/캐주얼/SF/수채). 비픽셀은 "배경" 키워드로 장면과 함께
  // 생성(서버 hasSceneDesc 매칭 → 불투명). 픽셀 1종은 adjustPrompt가 강제 투명화하므로 스프라이트로 둠.
  [
    "픽셀아트 도트 용사 캐릭터, 16비트 RPG 스타일, 정면 전신",
    "캐주얼 모바일게임풍 치비 마법사, 둥글둥글 귀여운 일러스트, 아늑한 마법 상점 배경",
    "SF 메카 파일럿, 디테일한 사이버펑크 일러스트, 격납고 배경",
    "꽃밭에 앉은 동양풍 소녀, 부드러운 수채 일러스트, 만개한 봄꽃 정원 배경",
  ],
  // 배경 — 장면 일러스트. 모두 장면 키워드(배경/던전/하늘/네온) 포함 → 서버가 투명화 안 함.
  // ※ 픽셀 장면은 "픽셀아트/도트" 대신 "16비트/픽셀"로 표기 — 그 토큰은 강제 투명 분기를 탐.
  [
    "레트로 16비트 픽셀 던전 배경, 횃불 불빛, 게임 배경 일러스트",
    "로우폴리 3D 스타일 외딴 섬 배경, 밝은 캐주얼 게임 풍경, 하늘과 바다",
    "카툰풍 평화로운 마을 배경, 밝은 셀 셰이딩 일러스트, 맑은 하늘",
    "비 내리는 네온 사이버펑크 도시 거리, 시네마틱 배경 일러스트, 젖은 노면 반사광",
  ],
  // 이펙트 — VFX 오버레이. 장면 키워드 없음 → 서버가 투명 배경으로 추출.
  [
    "도트 픽셀 폭발 이펙트, 16비트 게임 VFX, 강렬한 불티",
    "셀 애니메이션풍 칼날 슬래시 이펙트, 역동적인 VFX, 속도감 있는 궤적",
    "푸른 회복 마법 오라, 빛나는 입자 이펙트, 환상적인 일러스트",
    "흩날리는 벚꽃잎과 바람 이펙트, 부드러운 일러스트, 봄 감성",
  ],
  // 오브젝트 — 아이템 아이콘. "배경" 단어 없이 → 투명 배경으로 추출.
  // ※ "투명배경"이라 쓰면 "배경"이 장면으로 오인돼 오히려 불투명해짐 — 절대 쓰지 말 것.
  [
    "게임 아이템 아이콘 세트, 포션·검·방패, 깔끔한 일러스트",
    "픽셀아트 보물상자, 16비트 게임 아이템, 반짝이는 금화",
    "카툰풍 황금 코인, 반짝이는 게임 아이템 아이콘, 둥근 셀 셰이딩",
