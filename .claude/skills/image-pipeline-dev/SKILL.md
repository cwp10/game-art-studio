...
## 프롬프트 작성 규칙 (gpt-image-2 공식 패턴)

gpt-image-2는 CFG 기반 네거티브 프롬프트가 없다. 모든 제약은 단일 텍스트 블록 안에 넣어야 한다.
...
### 핵심 패턴: 긍정 진술 먼저 → 구체적 Avoid 열거

❌ 나쁜 예 (순수 부정 나열):
> no bad anatomy, no weird hands, no ugly face

✅ 좋은 예 (긍정 + Avoid):
> Use natural human anatomy, exactly two hands with five fingers each, relaxed natural pose.
> Avoid extra fingers, merged fingers, distorted hands, duplicated limbs, or unnatural joints.

규칙: "NEVER / DO NOT / NO ~" 표현 대신 → 목표 상태를 먼저 서술하고 `Avoid ~`로 구체적 금지사항을 열거한다.

### 기본 Constraints (모든 프롬프트에 포함)
...
- Avoid extra frames overlapping cell boundaries
```

### 이 파일의 walkCycleRule / singleDirWalkDir 등 프롬프트 블록도 동일 패턴을 따른다

`src/lib/mcp/handlers/shared.ts`의 모든 지시문은 긍정+Avoid 패턴으로 작성되어 있다. 새 규칙을 추가할 때도 이 패턴을 유지한다.
