...
description: image-generator 프로젝트에서 기능 개발·버그 수정·리팩터링을 조율하는 오케스트레이터. 이미지 파이프라인(codex/sharp/스프라이트시트 후처리), 풀스택 경계면(Next API/MCP/DB/React), 시각·통합 검증을 가로지르는 작업을 받으면 사용. "기능 추가", "버그 수정", "이미지 생성/편집/스프라이트시트 작업", "API/UI/DB 변경"에 적용. 단일 파일·단일 함수 변경, 설명 요청, 질문은 직접 응답 가능.
---
...
- `_workspace/` 없음 → **초기 실행**
- `_workspace/` 있음 + 이전 결과 수정·보완·재실행 요청 → **부분 재실행** (해당 영역 에이전트만 재호출, 이전 요약 전달)
...
경량 경로에서 visual-qa는 codex 생성 전에 **단위 테스트를 먼저 실행**한다 (`test-directions.ts`, `test-classify.ts` — codex 없이 빠름). 통과 후 필요 시 codex 생성 검증.

**표준 경로** (pipeline-engineer/fullstack-engineer + visual-qa):
