...
| 모션 페이즈 가이드 | **있다** (`RUN_PHASE_CYCLE`·`draw_motion_phase`) | **완료(옵트인)** — `motion-phase.ts`. 타원만 픽셀 동일 아님 |
...
**모션 페이즈 가이드 — 이식했다 (2026-08-16).** `RUN_PHASE_CYCLE`·`leg_points`·
`draw_motion_phase`·`state_motion_phases` 를 `src/lib/sprite/motion-phase.ts` 로 옮겼다.
정본과 같이 옵트인이고(`pnpm gen:sprite-run --motion-phase-guides`), 8프레임 로코모션
상태가 아니면 켜도 아무것도 그리지 않는다.

PIL 래스터화도 함께 이식했다 — 굵은 선은 사각형 폴리곤 스캔라인 채우기다. **꼭짓점이
스캔라인에 정확히 걸릴 때 교점이 두 번 세어져 그 줄이 통째로 비는 버그**를 반열림 y 구간으로
고쳤다(실측: 폭 5 대각선의 y=6 이 7px → 1px 였다). PIL 실측 행별 x 범위 32줄을 픽스처로
박아 잠갔다.

**타원만 픽셀 동일이 아니다 — 이식한 렌더러 중 유일하다.** PIL 은 타원을 각도 샘플
폴리곤으로 그려서 지름 29 원의 꼭대기 한 줄이 1px 이 아니라 **7px 로 평평하다**(여러 각도가
같은 y 로 반올림된다). 그 반올림 규칙을 재현하지 못했다 — round/trunc/floor 조합 ×
1도·0.5도 스텝 × 폴리라인·스팬 렌더를 원본 덤프에 맞춰 봤으나 어느 것도 일치하지 않았다.
우리 링이 한 픽셀 두껍고, 원본이 그리는 픽셀을 빠뜨리지는 않는다(항상 상위집합).
잔차 **528/524288 px(0.101%)이고 전부 머리 타원**이다. 선·팔·척추·접지선은 바이트
동일이고, 페이즈를 끈 가이드도 바이트 동일이다. 테스트가 상한과 "차이는 타원뿐" 을 잠근다.

프롬프트의 Motion phase requirements 는 **실제로 그렸을 때만** 붙는다 — 그리지도 않고
"가이드의 힌트를 따르라" 고 말하면 없는 것을 가리키는 지시가 된다.

**남긴 것 — 좌우 쌍 순서.** `canUsePlanDrivenPath` 가 단일 방향만 받으므로 현재 UI 에서
도달 불가다. 다방향을 열 때 45도 어휘 구멍(아래)과 함께 다룬다.

**UI 노출 없음**: `motionPhaseGuides` 는 request 필드 + CLI 플래그까지다. 정본도 CLI
플래그이므로 같은 위치지만, 우리 패널에서는 켤 수 없다. 패널 → 채팅 메시지 → MCP 도구
인자까지 새 필드를 관통시켜야 하고, 정본이 "명시적 실험에만" 이라고 못박은 기능이라
그 비용을 지금 쓰지 않았다.
