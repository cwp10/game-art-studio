...
| 2026-07-07 | advisor() 잔재 제거 → FAIL 2회 시 사용자 보고로 단일화; model 옵션 규칙을 에이전트 정의 위임으로 단순화; 파일 맵 현행화 (handlers/shared.ts·chroma-key.ts·sprite-frame-patch.ts, MCP 도구 13종, canvas-edit/composite-ai/nine-slice-trim 라우트, CanvasEditor 통합·useZoomPan/useStreamChat); visual-qa 게이트에 pnpm test/test:post·달리기 회귀 스크립트·qa-mcp-spritesheet 반영; Electron을 fullstack-engineer 관할로 편입; runImageTool 중복 제거(server.ts가 handlers/shared.ts에서 import); eslint에 dist/** ignore 추가로 lint 게이트 복구; _workspace_archive → docs/harness-archive/ 이전 + gitignore | imggen-feature-dev, agents/*.md, 서브 스킬 3종, src/lib/mcp/server.ts, eslint.config.mjs, .gitignore, CLAUDE.md | 2026-07-07 하네스 감사 결과 반영 |
